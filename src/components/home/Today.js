...
const Today = (props) => {
  ...
  const addFavorite = () => {
    dispatch(favoritesActions.addCity({name: mainCityName , weatherNow: weatherNow , fiveDays: fiveDays , gif: gif}))
  ...
  return (
      <div className="grid sm:grid-cols-1 md:grid-cols-2">
        <TodayUI gif={props.gif} mainCity={props.mainCity} temperatureC={props.temperatureC} temperatureF={props.temperatureF} WeatherText={props.WeatherText}  />
        
        <div>
          {!(!buttonStatus && nightMode) && (
          <button  ><img src={imgsrc}  alt={btnText} onClick={() => addFavorite()} /></button>
                                    
          )}
          {(!buttonStatus && nightMode) && (
          <button  ><img src={"https://img.icons8.com/pastel-glyph/128/FFFFFF/hearts--v1.png"}  alt={btnText} onClick={() => addFavorite()} /></button>
                                    
          )}
          <br />
          <div className={btnTextClass}>
            {btnText}
          </div>
        </div>
